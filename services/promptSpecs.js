...
- GOOGLE AI STUDIO — "get your first Gemini API key" (this EXACT flow, verified live):
  ...
  (8) copy the new key — action "click", targetType "icon", "targetLabel": "",
      ...
      dialog, the WRONG control). "advanceOnAnyClick": true, "screenRegion":"dialog",
      "anchorText":"API key", "anchorPosition":"right",
      "elementDescription": "the copy button on the same row as the generated API key
      (the long key string), to the RIGHT of the key — NOT the project-name copy icon
      at the top, NEVER the X / Close button", instruction "Click Copy next to your new
      API key." The key has generated by now, so vision can point at it; the copy
      control sits on the key's row lower in the dialog.
