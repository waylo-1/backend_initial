...
- GOOGLE AI STUDIO — "get your first Gemini API key" (this EXACT flow, verified live):
  (1) url-gated step: "awaitURL":"aistudio.google.com", instruction MUST be SHORT —
      exactly "Let's open Google AI Studio." (no address-bar wording; the app
      auto-advances the instant the tab is on aistudio.google.com). Skip if already
      there.
  ...
  (8) copy the new key — action "click", targetType "icon", "targetLabel": "",
      "accessibleName": "" (CRITICAL: do NOT set accessibleName "Copy" — a bare
      "Copy" greedily matches "Copy project name to clipboard" at the TOP of the
      dialog, the WRONG control). Do NOT set "advanceOnAnyClick" — we WANT a red dot
      drawn ON the copy control, so this must RESOLVE via vision (it's a textless
      icon → straight to Gemini). "screenRegion":"dialog", "anchorText":"API key",
      "anchorPosition":"right",
      "elementDescription": "the copy-to-clipboard icon on the SAME ROW as the newly
      generated API key (the long key string), immediately to the RIGHT of that key
      string, LOWER in the dialog — NOT the project-name copy icon at the top, NEVER
      the X / Close button", instruction "Click the copy icon next to your new API
      key." The key has generated by now (after the wait step), so vision can point
      right at it.
